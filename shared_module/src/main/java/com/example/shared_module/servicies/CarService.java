...
import com.example.shared_module.repositories.CarRepository;
...
public class CarService extends ItemService<CarDTO, Car> {
    ...
    @Override
    public ResponseEntity<?> add(CarDTO carDTO, boolean shouldTrigger, User user) {
        ...
        Car car = new Car();
        car.setAuthor(user.getUsername());
        car.setCool(carDTO.isCool());
        car.setName(carDTO.getName());
        Car savedCar = repository.save(car);
        if (shouldTrigger){
            simpMessagingTemplate.convertAndSend("/topic/cars", userService.getSocketMessage());
        }
        resp = auditService.doCommit(savedCar.getId(), EntityType.CAR, "Create", shouldTrigger, user);
        if (resp.getStatusCode() != HttpStatus.OK) {
            throw new DataIntegrityViolationException((String) resp.getBody());
        }
    ...
    @Override
    public ResponseEntity<?> addAll(Map<Integer, CarDTO> carDTOs, User user) {
        ...
        ResponseEntity<?> resp = auditService.doCommits(savedCar.stream().map(Car::getId).collect(Collectors.toList()), EntityType.CAR, "Create", user);
        if (resp.getStatusCode() != HttpStatus.OK) {
            return resp;
        }
    ...
    public ResponseEntity<?> update(Integer id, CarDTO carDTO) {
        ...
        try{
            ...
            resp = auditService.doCommit(updatedCar.getId(), EntityType.CAR, "Update", true, userService.getCurrentUser());
            if (resp.getStatusCode() != HttpStatus.OK) {
                return resp;
            }
