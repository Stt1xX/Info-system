...
import com.example.shared_module.repositories.CoordinatesRepository;
...
public class CoordinatesService extends ItemService<CoordinatesDTO, Coordinates> {
    ...
    @Override
    public ResponseEntity<?> add(CoordinatesDTO coordinatesDTO, boolean shouldTrigger, User user) {
        ...
        coordinates.setAuthor(user.getUsername());
        coordinates.setX(coordinatesDTO.getX());
        coordinates.setY(coordinatesDTO.getY());
        Coordinates savedCoordinates = repository.save(coordinates);
        if (shouldTrigger){
            simpMessagingTemplate.convertAndSend("/topic/coordinates", userService.getSocketMessage());
        }
        resp = auditService.doCommit(savedCoordinates.getId(), EntityType.COORDINATES, "Create", shouldTrigger, user);
        if (resp.getStatusCode() != HttpStatus.OK) {
            return resp;
        }
    ...
    @Override
    public ResponseEntity<?> addAll(Map<Integer, CoordinatesDTO> coordinatesDTOs, User user){
        ...
        ResponseEntity<?> resp = auditService.doCommits(savedCoordinates.stream().map(Coordinates::getId).collect(Collectors.toList()), EntityType.COORDINATES, "Create", user);
        if (resp.getStatusCode() != HttpStatus.OK) {
    ...
    public ResponseEntity<?> update(Integer id, CoordinatesDTO coordinatesDTO) {
        ...
        try{
            ...
            resp = auditService.doCommit(updatedCoordinates.getId(), EntityType.COORDINATES, "Update", true, userService.getCurrentUser());
            if (resp.getStatusCode() != HttpStatus.OK) {
                return resp;
            }
