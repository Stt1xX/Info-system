...
public class BusinessLogicService {
    ...
    public ResponseEntity<?> setLongingMood() {
        int n = humanService.getAll()
                ...
                .map(human -> {
                    ...
                    auditService.doCommit(human.getId(), EntityType.HUMAN, "Update", true, userService.getCurrentUser());
                    return 1;
                })
    ...
    public ResponseEntity<?> transferHeroes(){
        ...
        if (car == null) {
            ...
            auditService.doCommit(car.getId(), EntityType.CAR, "Create", true, userService.getCurrentUser());
        }
        Car finalCar = car;
        int n = humanService.getAll()
                .stream()
