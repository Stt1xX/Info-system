        ...
        <div class="form-group">
          <div class="flex flex-col space-y-2">
            <div v-for="option in options" :key="option" class="p-2 rounded flex items-center justify-between bg-gray-700">
              <span>{{ option }}</span>
              <div class="flex">
                <span @click="selectDirection('up', option)" :class="{'bg-gray-600': selectedOption === option && selectedDirection === 'up'}" class="ml-2 hover:bg-gray-600 w-8 h-8 flex items-center justify-center rounded-full cursor-pointer">
                  <svg fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="size-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75 12 3m0 0 3.75 3.75M12 3v18" />
                  </svg>
                </span>
                <span @click="selectDirection('down', option)" :class="{'bg-gray-600': selectedOption === option && selectedDirection === 'down'}" class="ml-2 hover:bg-gray-600 w-8 h-8 flex items-center justify-center rounded-full cursor-pointer">
                  <svg fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="size-6">
...
const selectedDirection = ref('up');
...
const applySort = () => {
  emit('apply', {option: selectedOption.value, direction: selectedDirection.value});
  closeMenu();
};
